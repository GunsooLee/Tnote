...
df_tnote = pd.DataFrame(np.array([
    ...
    ,
    ['화자3', '요즘 또 트렌드가 빅데이터 쪽이 트렌드도 많고 저희 이제 자격증 같은 것도 이제 빅데이터 쪽 관련된 자격증이 많이 나오고 있거든요.  그래서 저희 그런 자격증 취득하면서 연구 목적으로 빅데이터 쪽 하면은
 괜찮을 것 같습니다. AI를 한다 그러면 지금 아까 얘기한 것처럼 DPT도 있고 좀 분야가 많은 것 같은데 AI에서 어떤 분야가 좀 더해야 될지. 이거 조금 이거를 좀 정해야 될 것 같습니다. 제가 프로젝트 학교 다닐 때 썼던 게
 이제 AI 쪽 챗봇을 한번 쓴 적이 있었거든요.  그래서 근데 그거는 이제 AI라기보다는저희가 케이스 바이 케이스를 많이 만들어가지고 이제 사용자가 입력한 그 핵심 단어만 딱 잡아가지고 그거에 관련된 거를 보여주는 챗봇>을 했던 적이 있거든요.  챗봇도 하면은 나쁘지 않을 것 같은데챗봇을 이제 지금 저희 사용자들이 물어봤을 때 이거를 답변을 해 주는 거를 하는 것도 나쁘지 않을 것 같습니다. 어쨌든 연구 목적이면은 크게 문제없지 않을까 싶은데 이건 단순하게 의견이라서 그냥 이런 것도 해봤다라고 얘기드린 거예요. 회의록 작성할 때 참고하면 괜찮은데.']
     ,
