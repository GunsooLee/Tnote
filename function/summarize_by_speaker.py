...
def summarize_by_speaker(df):
    speaker_groups = df.groupby('화자')['내용'].apply(lambda x: f"{x.name}: \"{' '.join(x)}\".")  # 화자별로 내용 결합
    speaker_summaries = {}
    ...
    for speaker, combined_text in speaker_groups.items():
        ...
        summary_ids = model.generate(inputs['input_ids'], num_beams=4, max_length=256, min_length=16, early_stopping=True)
